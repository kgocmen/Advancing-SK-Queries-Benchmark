...
class QueryGenerator:
    def __init__(
        ...
        output_dir: str = "./data/workloads/" + str(EXPERIMENT),
        ...
        self.radius = radius
        self.prepositions = [
            "in", "at", "on", "near", "by", "around", "to", "from", "with",
            "about", "inside", "outside", "between", "among", "across",
            "through", "over", "under"
        ]
    ...
    def _m2deg(self, meters: float) -> float:
        # crude lat-degree conversion; fine for small radii
    ...
    def _sql(self, lon, lat, key_or_pair, k):
        ...
        if isinstance(key_or_pair, (tuple, list)) and len(key_or_pair) == 2:
            ...
            rhs = json.dumps({key: val})                # {"key":"value"}
            predicate = f"tags @> '{rhs}'::jsonb"
        else:
            key = str(key_or_pair).replace("'", "''")   # key existence
    ...
    def produce_from_file(self, path):
        ...
        cont_by_k: Dict[int, List[str]] = {k: [] for k in self.k_values}   # contrastive
        ...
        for keyword, lat, lon in triples:
            ...
            zvec = self._contrastive_vec(keyword, lat, lon)
            z_str = _as_pgvector(zvec)
            ...
            for k in self.k_values:
                ...
                cont_by_k[k].append(self._c_sql(lon=lon, lat=lat, vec=z_str, k=k))
        ...
        for k, queries in cont_by_k.items(): self._write_sql(f"custom_contrastive_embedded_queries_k{k}.sql", queries)
    ...
    def generate_from_dataset(
        ...
        search: list = (
            "amenity","shop","name","cuisine","building","tourism",
            "leisure","highway","public_transport","office"
        )
    ):
        df = pd.read_csv(csv_file)
        df[TEXT_COL] = df[TEXT_COL].fillna("{}").apply(safe_parse_json)

        non_by_k: Dict[int, List[str]] = {k: [] for k in self.k_values}
        emb_by_k: Dict[int, List[str]] = {k: [] for k in self.k_values}
        conc_by_k: Dict[int, List[str]] = {k: [] for k in self.k_values}
        cont_by_k: Dict[int, List[str]] = {k: [] for k in self.k_values}

        idxs = df.index.to_list()
        n = min(point_count, len(df))
        for _ in range(n):
            pos_idx = random.choice(idxs)
            ref_lat = float(df.at[pos_idx, LAT_COL])
            ref_lon = float(df.at[pos_idx, LON_COL])

            # pick (key,value) from a random donor row
            kv = None
            while kv is None:
                donor_tags = df.at[random.choice(idxs), TEXT_COL]
            ...
            zvec = self._contrastive_vec(text, ref_lat, ref_lon); zvec_str = _as_pgvector(zvec)
    ...
    def _read_keyword_lat_lon(self, path: str) -> List[Tuple[str, float, float]]:
        ...
        def infer_coords_for_texts(texts: List[str]) -> List[Tuple[str, float, float]]:
            ...
            semantic_matrix = np.load(semantic_path)          # (N, D)
            poi_df = pd.read_csv(self.input_csv)
            poi_coords = poi_df[["lat", "lon"]].values        # (N, 2)

            out: List[Tuple[str, float, float]] = []
            for q in texts:
                short_q = self._extract_after_preposition(q)
                q_vec = generate_semantic_embedding_query(short_q).reshape(1, -1)
    ...
    def _contrastive_encoder_init(self, ckpt: str, proj_dim: int, w_text: float, w_spatial: float):
        ...
        self._c_tok = AutoTokenizer.from_pretrained(SEMANTIC)
...
if __name__ == "__main__":
    ...
    if ckpt_path == "./contrastive/contrastive_model.pt":
        ckpt_path = f"./contrastive/d{CONTRASTIVE['proj_dim']}.pt"

    # Initialize contrastive encoder once
