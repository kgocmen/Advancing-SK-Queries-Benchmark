...
# -------------------------
# Spatial Encoder (Sinusoidal only)
# -------------------------

class SinusoidalSpatial(nn.Module):
    def __init__(self, output_dim=128, hidden_dim=128, bands=(1.0, 2.0, 4.0, 8.0)):
        super().__init__()
    ...
    def forward(self, lonlat):
        lonlat = lonlat.float()
        lon_rad = torch.deg2rad(lonlat[..., 0]).unsqueeze(-1)
        lat_rad = torch.deg2rad(lonlat[..., 1]).unsqueeze(-1)
        lon_b = lon_rad * self._bands
        lat_b = lat_rad * self._bands
        feats = torch.cat([torch.sin(lon_b), torch.cos(lon_b),
                           torch.sin(lat_b), torch.cos(lat_b)], dim=-1)
...
class POIDataset(Dataset):
    def __init__(self, csv_path: str, max_len: int = 64):
        ...
        df["__text"] = df["__text"].apply(lambda s: s if isinstance(s, str) and s.strip() else "poi")
        self.df = df.reset_index(drop=True)
        self.tok = AutoTokenizer.from_pretrained(SEMANTIC)
    ...
    def __getitem__(self, idx):
        ...
        enc = self.tok(text, return_tensors="pt", padding="max_length",
...
def train(args):
    ...
    ds = POIDataset(args.csv, args.max_len)
...
def build_parser():
    p = argparse.ArgumentParser(description="Contrastive Text↔Location (Sinusoidal-only, fixed cols)")
    ...
    sp_tr.add_argument("--epochs", type=int, default=7)
    sp_tr.add_argument("--batch-size", type=int, default=256)
